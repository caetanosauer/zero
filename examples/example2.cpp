...
rc_t example_thread_t::do_init()
{
    ...
    if (nuke_it) {
        ...
        stid = stid_t (vid, FIRST_STORE_ID);
    }
...
rc_t example_thread_t::run_example() {
    ...
    smsize_t datalen = 4;
    bool found;
    W_DO(ss_m::find_assoc(stid, key, data, datalen, found));
    if (found) {
        std::cout << "Found! data was:" << std::string(data, datalen) << std::endl;
    } else {
        return RCOK;
    }
    
    std::cout << "updating data..." << std::endl;
    W_DO(ss_m::overwrite_assoc(stid, key, "b", 3, 1)); // overwrite 4th byte
    W_DO(ss_m::find_assoc(stid, key, data, datalen, found));
    if (found) {
        std::cout << "Found! data was:" << std::string(data, datalen) << std::endl;
    }
...
int main(int argc, char **argv) {
    ...
    example_thread_t t(argv[1], argv[2], false);
