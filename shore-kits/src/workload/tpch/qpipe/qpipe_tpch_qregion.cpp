...
/** @file:   qpipe_qregion.cpp
 *
 *  @brief:  Implementation of a region tablescan over QPipe over Shore-MT
 *
...
/********************************************************************
 *
 * QPIPE Scan Region - Structures needed by operators
 *
 ********************************************************************/
...
public:
    ...
    tscan_filter_t(ShoreTPCHEnv* tpchdb, qregion_input_t &in)
          ...
          //: tuple_filter_t(sizeof(tpch_lineitem_tuple)), _tpchdb(tpchdb)
    {
        ...
        _prnat = _tpchdb->region_man()->get_tuple();
        _rr.set_ts(_tpchdb->region_man()->ts(),
                   _tpchdb->region_desc()->maxsize());
        _prnat->_rep = &_rr;
    ...
    ~tscan_filter_t()
    {
        ...
        _tpchdb->region_man()->give_tuple(_prnat);
    }
    ...
    bool select(const tuple_t &input) {
        ...
        if (!_tpchdb->region_man()->load(_prnat, input.data)) {
    ...
    void project(tuple_t &d, const tuple_t &s) {
        ...
        tpch_region_tuple *dest;
        dest = aligned_cast<tpch_region_tuple>(d.data);

        _prnat->get_value(0,  _region.R_REGIONKEY);
        _prnat->get_value(1,  _region.R_NAME, 25);
        _prnat->get_value(2,  _region.R_COMMENT, 25);

        memcpy(dest,&_region,sizeof(tpch_region_tuple));
        TRACE(TRACE_RECORD_FLOW, "%d %s\n", dest->R_REGIONKEY, dest->R_NAME);
    }
...
class tpch_region_process_tuple_t : public process_tuple_t
{
public:

    void begin() {
        TRACE(TRACE_QUERY_RESULTS, "*** SCAN REGION ANSWER ...\n");
    ...
    static const c_str* dump_tuple(tuple_t* tup) {
        tpch_region_tuple *dest;
        dest = aligned_cast<tpch_region_tuple> (tup->data);
        return new c_str("%d|%s|%s|\n",
	  dest->R_REGIONKEY,
	  dest->R_NAME,
	  dest->R_COMMENT);
    }
...
/********************************************************************
 *
 * QPIPE Scan Region - Packet creation and submission
 *
 ********************************************************************/

w_rc_t ShoreTPCHEnv::xct_qpipe_qregion(const int xct_id, qregion_input_t& in)
{
    TRACE( TRACE_ALWAYS, "********** SCAN REGION *********\n");
    ...
    tuple_fifo* tscan_out_buffer =
        new tuple_fifo(sizeof(tpch_region_tuple));
    tscan_packet_t* tscan_packet =
        new tscan_packet_t("TSCAN REGION",
                           tscan_out_buffer,
                           new tpch_qregion::tscan_filter_t(this,in),
                           this->db(),
                           _pregion_desc.get(),
                           pxct
                           /*, SH */
    ...
    tuple_fifo* fdump_output = new tuple_fifo(sizeof(tpch_region_tuple));
    fdump_packet_t* fdump_packet =
            new fdump_packet_t(c_str("FDUMP"),
            fdump_output,
            new trivial_filter_t(fdump_output->tuple_size()),
            NULL,
            c_str("%s/region.tbl", getenv("HOME")),
            NULL,
            tscan_packet,
	    tpch_qregion::dump_tuple);


    // AGG PACKET CREATION
    tuple_fifo* count_output_buffer =
        new tuple_fifo(sizeof(tpch_qregion::count_tuple));
    packet_t* count_packet =
        new partial_aggregate_packet_t("COUNT",
                                       count_output_buffer,
                                       new trivial_filter_t(count_output_buffer->tuple_size()),
                                       fdump_packet,
                                       new tpch_qregion::count_aggregate_t(),
                                       new tpch_qregion::count_aggregate_t::count_key_extractor_t(),
    ...
    // Dispatch packet
    tpch_qregion::tpch_region_process_tuple_t pt;
