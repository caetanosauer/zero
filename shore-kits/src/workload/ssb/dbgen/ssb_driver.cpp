...
load_dists (void)
{
  read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"p_cntr", &p_cntr_set);
  read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"colors", &colors);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"p_types", &p_types_set);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"nations", &nations);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"regions", &regions);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"o_oprio",
		&o_priority_set);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"instruct",
		&l_instruct_set);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"smode", &l_smode_set);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"category",
		&l_category_set);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"rflag", &l_rflag_set);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"msegmnt", &c_mseg_set);

	/* load the distributions that contain text generation */
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"nouns", &nouns);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"verbs", &verbs);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"adjectives", &adjectives);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"adverbs", &adverbs);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"auxillaries", &auxillaries);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"terminators", &terminators);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"articles", &articles);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"prepositions", &prepositions);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"grammar", &grammar);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"np", &np);
	read_dist (env_config ((char*)DIST_TAG, (char*)DIST_DFLT), (char*)"vp", &vp);
		...
		case ORDER:
		  ...
		  if (insert_segments  && (upd_num > 0))
			...
			if (set_seeds == 0)
                          {
				if (validate)
					tdefs[tnum].verify((void*)&o, 0);
				else
                                  tdefs[tnum].loader[direct] ((void*)&o, upd_num);
                          }
			break;
		case SUPP:
			mk_supp (i, &supp);
			if (set_seeds == 0)
                          {
				if (validate)
					tdefs[tnum].verify((void*)&supp, 0);
				else
					tdefs[tnum].loader[direct] ((void*)&supp, upd_num);
                          }
			break;
		case CUST:
			mk_cust (i, &cust);
			if (set_seeds == 0)
                          {
				if (validate)
					tdefs[tnum].verify((void*)&cust, 0);
				else
					tdefs[tnum].loader[direct] ((void*)&cust, upd_num);
                          }
			break;
#ifdef SSBM
		case PART:
		...
		case PART:
  		case PART_PSUPP:
			...
			if (set_seeds == 0)
                          {
				if (validate)
					tdefs[tnum].verify((void*)&part, 0);
				else
					tdefs[tnum].loader[direct] ((void*)&part, upd_num);
                          }
			break;
#ifdef SSBM
		case DATE:
			...
			if (set_seeds == 0)
                          {
				if (validate)
					tdefs[tnum].verify((void*)&dt, 0);
				else
					tdefs[tnum].loader[direct] ((void*)&dt, 0);
                          }
			break;
#else
		case NATION:
			...
			if (set_seeds == 0)
                          {
				if (validate)
					tdefs[tnum].verify((void*)&code, 0);
				else
					tdefs[tnum].loader[direct] ((void*)&code, 0);
                          }
			break;
		case REGION:
			mk_region (i, &code);
...
process_options (int count, char **vector)
{
	...
	while ((option = getopt (count, vector,
                                 (char*)"b:C:Dd:Ffi:hn:O:P:qr:s:S:T:U:v")) != -1)
          { 
