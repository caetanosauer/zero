...
long      weight,
    ...
    if (d_path == NULL)
		{
		sprintf(line, "%s%c%s", 
			env_config((char*)CONFIG_TAG, (char*)CONFIG_DFLT), PATH_SEP, path);
		fp = fopen(line, "r");
		OPEN_CHECK(fp, line);
		}
	else
    ...
    while (fgets(line, sizeof(line), fp) != NULL)
        ...
        if (!name_set)
            ...
            {
            if (!dssncasecmp(line, (char*)"END", 3))
                {
                fclose(fp);
                return;
                }
        ...
        if (!dsscasecmp(token, (char*)"count"))
            {
            target->count = weight;
            target->list =
                (set_member *)
...
tbl_open(int tbl, char *mode)
{
    ...
    if (*tdefs[tbl].name == PATH_SEP)
        ...
        sprintf(fullpath, "%s%c%s",
            env_config((char*)PATH_TAG, (char*)PATH_DFLT), PATH_SEP, tdefs[tbl].name);
