...
/** @file shore_desc_sort_buf.cpp
 *
 *  @brief Implementation of comparison functions for desc_sort_buffer_t 
 *
...
void desc_sort_man_impl::sort()
{
    ...
    switch (_ptable->desc(0)->type()) {
    case SQL_BIT:
        qsort(_sort_buf, _tuple_count, _tuple_size, compare_bit_desc); break;
    case SQL_SMALLINT:
        qsort(_sort_buf, _tuple_count, _tuple_size, compare_smallint_desc); break;
    case SQL_INT:
        qsort(_sort_buf, _tuple_count, _tuple_size, compare_int_desc); break;
    case SQL_LONG:
            qsort(_sort_buf, _tuple_count, _tuple_size, compare_long_desc); break;
    case SQL_FLOAT:
            qsort(_sort_buf, _tuple_count, _tuple_size, compare_double_desc); break;
    case SQL_FIXCHAR:
            qsort(_sort_buf, _tuple_count, _tuple_size, compare_fixchar_desc); break;
    default: 
        /* not implemented yet */
        assert(0);
    }
...
w_rc_t desc_sort_man_impl::get_sort_iter(ss_m* db,
                                    	desc_sort_iter_impl* &sort_iter)
{
    sort_iter = new desc_sort_iter_impl(db, _pspecifictable, this);
    return (RCOK);
}
