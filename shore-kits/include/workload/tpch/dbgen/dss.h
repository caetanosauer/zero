...
#ifndef DBGEN_TPCH_DSS_H
#define DBGEN_TPCH_DSS_H
...
typedef struct
{
  ...
  DSS_HUGE      base;
  int       (*header) (FILE* f);
  int       (*loader[2]) (void* o, int mode);
  long      (*gen_seed)(int child, DSS_HUGE skip_count);
  int       (*verify) (void* vo, int mode);
  int       child;
  DSS_HUGE vtotal;
}         tdef;
...
FILE	*tbl_open PROTO((int tbl, const char *mode));
...
#define  CONFIG_DFLT "include/workload/tpch/dbgen"			/* default directory to config files */
