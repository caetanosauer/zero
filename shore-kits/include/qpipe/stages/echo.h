...
#ifndef __QPIPE_ECHO_H
#define __QPIPE_ECHO_H
...
struct echo_packet_t : public packet_t {
    ...
    echo_packet_t(const c_str       &packet_id,
                  tuple_fifo*        output_buffer,
                  tuple_filter_t*    output_filter,
                  packet_t*          input)
	: packet_t(packet_id, PACKET_TYPE, output_buffer, output_filter,
                   create_plan(output_filter, input->plan()),
    ...
    static query_plan* create_plan(tuple_filter_t* filter, query_plan const* child)
    {
        c_str action("%s", PACKET_TYPE.data());
...
class echo_stage_t : public stage_t {

public:

    static const c_str DEFAULT_STAGE_NAME;
    typedef echo_packet_t stage_packet_t;
