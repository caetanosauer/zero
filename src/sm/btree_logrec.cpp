...
DBGOUT3( << "&&&& UNDO insertion, key: " << key);
    ...
    rc_t rc = smlevel_2::bt->remove_as_undo(stid_t(header._vid, header._snum), key);
    if(rc.is_error())
    {
...
btree_update_log::undo(fixable_page_h*)
{
    ...
    // ***LOGICAL*** don't grab locks during undo
    rc_t rc = smlevel_2::bt->update_as_undo(stid_t(header._vid, header._snum), key, old_el);
    if(rc.is_error()) {
        W_FATAL(rc.err_num());
    }
}
...
void btree_overwrite_log::undo(fixable_page_h*)
{
    ...
    // ***LOGICAL*** don't grab locks during undo
    rc_t rc = smlevel_2::bt->overwrite_as_undo(stid_t(header._vid, header._snum), key, old_el, offset, elen);
    if(rc.is_error()) {
        W_FATAL(rc.err_num());
    }
}
...
DBGOUT3( << "&&&& UNDO deletion by remove ghost mark, key: " << key);
        ...
        rc_t rc = smlevel_2::bt->undo_ghost_mark(stid_t(header._vid, header._snum), key);
        if(rc.is_error()) {
