...
{
    FUNC(btree_impl::_sx_rebalance_blink);
    sys_xct_section_t sxs;
    W_DO(sxs.check_error_on_start());
    rc_t ret = _ux_rebalance_blink_core(page);
    W_DO (sxs.end_sys_xct (ret));
    return ret;
}

rc_t btree_impl::_ux_rebalance_blink_core(btree_p &page)
{
    ...
    smsize_t balanced_size = (used + blink_used) / 2 + SM_PAGESIZE / 10;
    
    int move_count = 0;
    // worth rebalancing?
    if (used < blink_used * 3) {
    ...
    return _ux_rebalance_blink_core (page, blink_p, move_count);
}

rc_t btree_impl::_ux_rebalance_blink_core(btree_p &page, btree_p &blink_p, int32_t move_count)
{
    w_assert1 (page.latch_mode() == LATCH_EX);
    w_assert1 (blink_p.latch_mode() == LATCH_EX);
    w_assert1 (page.get_blink() == blink_p.pid().page);
    if (move_count == 0) {
        return RCOK;
    }
    ...
    W_DO (log_btree_blink_rebalance (blink_p, page.pid().page, move_count));
    ...
    if (blink_p.is_node()) {
        ...
        W_DO(blink_p.format_steal(scratch_p.pid(), scratch_p.btree_root(), scratch_p.level(), new_pid0,
            scratch_p.get_blink(),
        ...
        W_DO(blink_p.format_steal(scratch_p.pid(), scratch_p.btree_root(), scratch_p.level(), 0,
            scratch_p.get_blink(),
            new_low_key, high_key, chain_high_key,
            false, // don't log it
    ...
    w_assert3(blink_p.is_consistent(true, true));

    return RCOK;
}

rc_t btree_impl::_sx_merge_blink(btree_p &page)
{
    FUNC(btree_impl::_sx_merge_blink);
    sys_xct_section_t sxs;
    W_DO(sxs.check_error_on_start());
    rc_t ret = _ux_merge_blink_core(page);
    W_DO (sxs.end_sys_xct (ret));
    return ret;
}
...
rc_t btree_impl::_ux_merge_blink_core(btree_p &page)
{
    w_assert1 (xct()->is_sys_xct());
    w_assert1 (page.latch_mode() == LATCH_EX);
    if (page.get_blink() == 0) {
    ...
    size_t additional = estimate_required_space_to_merge(page, blink_p);
    ...
    W_DO(log_btree_blink_merge (page));
    W_DO(blink_p.set_tobedeleted(true)); // log it (for foster-child)
    ...
    blink_p.copy_fence_high_key(high_key); // after merging, foster-child's high-key is high-key
    if (blink_p.get_blink() != 0) {
        // if no blink after merging, chain-high will disappear
    ...
    W_DO(page.format_steal(scratch_p.pid(), scratch_p.btree_root(), scratch_p.level(), scratch_p.pid0(),
        ...
        &blink_p, 0, blink_p.nrecs() // steal from foster-child
    ));
    w_assert3(page.is_consistent(true, true));
...
rc_t btree_impl::_sx_deadopt_blink(btree_p &real_parent, slotid_t foster_parent_slot)
{
    FUNC(btree_impl::_sx_deadopt_blink);
    sys_xct_section_t sxs;
    W_DO(sxs.check_error_on_start());
    rc_t ret = _ux_deadopt_blink_core(real_parent, foster_parent_slot);
    W_DO (sxs.end_sys_xct (ret));
    return ret;
}

void btree_impl::_ux_deadopt_blink_apply_real_parent(btree_p &real_parent,
    ...
    shpid_t foster_child_id, const w_keystr_t & W_IFDEBUG1(low_key), const w_keystr_t &high_key)
{
    ...
    w_assert1 (foster_parent.get_blink() == 0);
    ...
    foster_parent._pp->btree_blink = foster_child_id;
}
rc_t btree_impl::_ux_deadopt_blink_core(btree_p &real_parent, slotid_t foster_parent_slot)
{
    w_assert1 (xct()->is_sys_xct());
    w_assert1 (real_parent.is_node());
    w_assert1 (real_parent.latch_mode() == LATCH_EX);
    ...
    W_DO(log_btree_blink_deadopt_real_parent(real_parent, foster_child_id, foster_parent_slot));
    W_DO(log_btree_blink_deadopt_foster_parent(foster_parent, foster_child_id, low_key, high_key));
    
    _ux_deadopt_blink_apply_real_parent (real_parent, foster_child_id, foster_parent_slot);
    _ux_deadopt_blink_apply_foster_parent (foster_parent, foster_child_id, low_key, high_key);
