    ...
    const cvec_t&        el)
{
    ...
    if (!found) {
        ...
        Logger::log<btree_insert_nonghost_log>(&leaf, key, el, false /*is_sys_txn*/);
...
rc_t btree_impl::_ux_reserve_ghost_core(btree_page_h &leaf, const w_keystr_t &key, int elem_len) {
    ...
    Logger::log<btree_ghost_reserve_log> (&leaf, key, elem_len);
...
btree_impl::_ux_update_core(StoreID store, const w_keystr_t &key, const cvec_t &el)
{
    ...
    Logger::log<btree_update_log> (&leaf, key, old_element, old_element_len, el);
        ...
        const char *el, smsize_t offset, smsize_t elen)
{
    ...
    Logger::log<btree_overwrite_log> (&leaf, key, old_element, el, offset, elen);
...
DBGOUT3( << "&&&& _ux_remove_core - not found");
    ...
    else
    {
        ...
        Logger::log<btree_ghost_mark_log> (&leaf, slots, false /*is_sys_txn*/);
