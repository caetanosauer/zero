    ...
    const cvec_t&        el)
{
    ...
    if (found) {
        ...
        if (need_lock) {
            W_DO (_ux_lock_key(leaf, key, LATCH_EX, create_part_okvl(w_okvl::X, key, el), false));
            alreay_took_XN = true;
        }
        ...
        const char *el, smsize_t offset, smsize_t elen)
{
    ...
    if (need_lock) {
        W_DO (_ux_lock_key(leaf, key, LATCH_EX, create_part_okvl(w_okvl::X, key, el + offset, elen), false));
    }
