...
scan_file_i::scan_file_i(
        const stid_t& stid_, const rid_t& start,
         ...
         okvl_mode::singular_lock_mode /*mode TODO: remove.  is documented as ignored*/,
        const bool  bIgnoreLatches) 
: xct_dependent_t(xct()),
  stid(stid_),
  curr_rid(start),
...
scan_file_i::scan_file_i(const stid_t& stid_, concurrency_t cc, 
   bool pre, okvl_mode::singular_lock_mode /*mode TODO: remove. this documented as ignored*/,
   const bool  bIgnoreLatches)
: xct_dependent_t(xct()),
  stid(stid_),
  _eof(false),
