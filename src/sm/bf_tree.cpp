...
//          uint32_t ret = atomic_cas_32((uint32_t*) &(cb._pin_cnt), cur_ucnt, cur_ucnt + 1);
...
//    atomic_inc_32((uint32_t*) &(_control_blocks[idx]._pin_cnt));
...
// atomic_cas_32(target,comp,new)  always returns target
//        uint32_t old_pin_cnt = atomic_cas_32((uint32_t*) &(cb._pin_cnt), (uint32_t) 0, (uint32_t) -1);
...
//    atomic_inc_32 ((uint32_t*) &(_control_blocks[new_parent_idx]._pin_cnt));
    lintel::unsafe::atomic_fetch_add((uint32_t*) &(_control_blocks[new_parent_idx]._pin_cnt),1);
