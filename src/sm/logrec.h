...
struct prepare_lock_t  {
    ...
    okvl_mode::element_lock_mode mode; // for this group of locks
    uint32_t     num_locks; // in the array below
    enum            { max_locks_logged = (logrec_t::max_data_sz - sizeof(okvl_mode::element_lock_mode) - sizeof(uint32_t)) / sizeof(lockid_t) };
...
struct prepare_all_lock_t 
{
    ...
    struct LockAndModePair {
        lockid_t    name;
        okvl_mode::element_lock_mode mode; // for this lock
    };
    ...
    prepare_all_lock_t(uint32_t num, 
                       lockid_t *locks,
                       okvl_mode::element_lock_mode *modes) {
        num_locks = num;
        uint32_t i;
