...
LogArchiver::LogArchiver(const char* archdir, bool sort, size_t workspaceSize)
    ...
    startLSN(lsn_t::null), lastSkipLSN(lsn_t::null), totalAlloc(0),
    totalUsed(0), shutdown(false), control(&shutdown), sortArchive(sort),
    heap(heapCmp) 
{
...
/*
 * Empties the buffer and invoke defragmentation of the sort workspace
 * if a certain threshold is reached. In practice, the ratio of wasted
 * space is rarely over 5%, and so it is never invoked.
 */
void LogArchiver::checkDefrag()
{
    // This almost never happens (ratio usually > 95%)
    float ratio = (float) totalUsed / totalAlloc;
    DBGTHRD(<< "Fragmentation ratio is " << ratio);
    if (ratio <= DEFRAG_THRESHOLD) {
        // first write out all contents of the heap
        while (heap.NumElements() > 0) {
            selection();
        }
        // now invoke heap defragmentation (rebuild free lists)
        DBGTHRD(<< "Invoking heap defragmentation");
        W_COERCE(workspace->defrag());
    }
}
